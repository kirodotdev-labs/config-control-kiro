/**
 * @fileoverview Steering page for viewing and editing steering configuration files.
 */
...
import SteeringFileTree from './SteeringFileTree';
...
const Steering = () => {
  ...
  const { leftPanelWidth, isDragging, handleMouseDown } = useResizablePanels(33, 'steeringLeftPanelWidth', 20, 60, 'vertical');
  ...
  const saveFile = useCallback(async () => {
    ...
    queryClient.invalidateQueries(['steering-files']);
      ...
      <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden' }} data-resizable-container>
        <Box sx={{ width: `${leftPanelWidth}%`, borderRight: 1, borderColor: 'divider', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider', bgcolor: 'background.paper', flexShrink: 0 }}>
            <Typography variant="h6">Steering Files</Typography>
